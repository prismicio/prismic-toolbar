...
export function setup(portToMainWindow) {
	portToMainWindow.onmessage = async (msg) => {
		...
		const result /* Promise<Object> */ = await (() => {
			switch (type) {
				case Messages.PreviewState:
					return getPreviewState();
				case Messages.PredictionDocs:
					return getPredictionDocs(data);
				case Messages.DevModeQueriesResults:
					return getDevModeQueriesResults(data);
				case Messages.UpdatePreview:
					return updatePreview();
				case Messages.ClosePreviewSession:
					return closePreviewSession();
				case Messages.SharePreview:
					return sharePreview(data);
				case Messages.TrackDocumentClick:
					return trackDocumentClick(data);
				case Messages.TrackToolbarSetup:
					return trackToolbarSetup();
				default:
