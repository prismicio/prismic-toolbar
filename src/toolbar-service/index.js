	...
	establishConnection(/* HTMLIFrameElement */ iframe) /* Promise<MessagePort> */ {
		return new Promise((resolve) => {
			...
			if (iframe.contentWindow)
				iframe.contentWindow.postMessage(ToolbarServiceProtocol.SetupPort, "*", [portToMainWindow])
...
const Iframe = {
	...
	async setup() /* void */ {
		...
		window.addEventListener("message", Iframe.initialisationMessageHandler)
	},
	...
	initialisationMessageHandler(/* MessageEvent */ message) /* void */ {
		if (message.data === ToolbarServiceProtocol.SetupPort) {
			...
			window.removeEventListener("message", Iframe.initialisationMessageHandler)
			...
			Iframe.initializationDelayedTimeouts.push(
				setTimeout(() => {
