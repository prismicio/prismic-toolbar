...
const Client = {
	...
	establishConnection(
		...
		return new Promise((resolve) => {
			const { port1: portToIframe, port2: portToMainWindow } =
				new MessageChannel();
...
const Iframe = {
	...
	async setup() /* void */ {
		...
		window.addEventListener("message", (msg) =>
			Iframe.initialisationMessageHandler(msg),
		);
	},
	...
	initialisationMessageHandler(/* MessageEvent */ message) /* void */ {
		if (message.data === ToolbarServiceProtocol.SetupPort) {
			...
			Iframe.initializationDelayedTimeouts.forEach((timeout) =>
