...
const scriptURL = new URL(
	getAbsoluteURL(document.currentScript.getAttribute("src")),
);
const repoParam = scriptURL.searchParams.get("repo");
if (repoParam !== null) {
	repos = new Set([...repos, ...repoParam.split(",")]);
}
...
if (!repos.size) {
	warn`Your are not connected to a repository.`;
}
...
async function setup(rawInput) {
	...
	if (!domain) {
		return warn`
Failed to setup. Expected a repository identifier (example | example.prismic.io) but got ${
			rawInput || "nothing"
		}`;
	}
...
Already connected to a repository (${setupDomain}).`;
	...
	const previewCookieHelper = new PreviewCookie(
		previewState.auth,
	...
	const prediction =
		previewState.auth && new Prediction(toolbarClient, previewCookieHelper);
...
function parseEndpoint(repo) {
	...
	if (!/^(https?:\/\/)?[-a-zA-Z0-9.\/]+/.test(repo)) {
		return null;
	}
	// eslint-disable-next-line no-undef
	if (!repo.includes(".")) {
		repo = `${repo}.prismic.io`;
	}
