...
export function startDocumentHeightReporting({ parentOrigin }: { parentOrigin: string }) {
	if (document.body) start(parentOrigin)
	else window.addEventListener("DOMContentLoaded", () => start(parentOrigin))
}

function start(parentOrigin: string) {
	let lastMeasure: { height: number; viewportHeight: number; viewportWidth: number } | undefined
	let heightToPost: number | undefined
	let viewportDrivenGrowths = 0
	let isViewportDriven = false

	const post = (height: number | null) =>
		window.parent.postMessage({ type: documentHeightMessageType, height }, parentOrigin)
	...
	const measure = () => {
		...
		if (previous) {
			...
			const isViewportDrivenGrowth =
				!isReflow &&
				viewportGrowth > 0 &&
				heightGrowth >= viewportGrowth * viewportDrivenGrowthRatio

			viewportDrivenGrowths = isViewportDrivenGrowth ? viewportDrivenGrowths + 1 : 0
