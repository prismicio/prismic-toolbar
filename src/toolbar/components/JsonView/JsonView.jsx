...
export class JsonView extends Component {
	...
	copyToClipboard = /* Object */ (node) => {
		...
		if (nodeCopied !== node) {
			if (nodeCopied.path) this.removeOldCopied(data, metadata, nodeCopied)
	...
	setIsCopied(/* Object */ json, /* List[String] */ keyNames, /* Boolean */ value) {
		...
		const reducer = (/* Object */ acc, /* String */ key, /* Int */ index) => /* Object */ {
		...
		const nodeToModify = keyNames
			.map((key) =>
				key[0] === "[" && key[key.length - 1] === "]" ? key.substring(1, key.length - 1) : key,
			)
			.reduce(reducer, json)
	...
	removeOldCopied = (/* Object */ data, /* Object */ metadata, /* Object */ nodeCopied) => {
	...
	decorators = {
		...
		Key: (props) => (
			...
			<span className={props.node.children ? "key-object" : "key-string"} onClick={props.onClick}>
		...
		Copy: (props) => (
			<span className="copy-button" onClick={() => this.copyToClipboard(props.node)}>
		...
		Container: (props) => {
			...
			return (
				<div className="json-view-container">
					...
					<Value node={props.node} onClick={props.onClick} stringCheck={stringCheck} />
