...
export class DevMode extends Component {
	...
	getGraphqlItemInfo(/* Object */ query) /* : String */ {
		...
		const title = entries.map((e) => e[0]).reduce((acc, val) => acc + " & " + val)
    ...
    */
	...
	countLinkedDocInDocument(/* Object */ data) /* : Int */ {
		...
		const count = Object.keys(data).reduce((/* Int */ acc, /* Object || String */ key) => {
	...
	render() {
		...
		return (
			<div>
				{apiQueries.map((query, queryIndex) => {
					// apiQuery : [Object]
					...
					return (
						<Collapsible
							key={queryIndex}
							trigger={<DevModeItem nbLinkedDoc={itemInfos.nbLinkedDoc} title={itemInfos.title} />}
							triggerWhenOpen={
								<DevModeItem isOpen nbLinkedDoc={itemInfos.nbLinkedDoc} title={itemInfos.title} />
							}
							transitionTime={100}
						>
							{query.map((doc) => (
				...
				{graphqlApiQueries.map((query, queryIndex) => {
					...
					return (
						<Collapsible
							key={queryIndex}
							trigger={<DevModeItem isGraphql maxStringSize={this.maxStringSize} title={title} />}
							triggerWhenOpen={
								<DevModeItem isGraphql isOpen maxStringSize={this.maxStringSize} title={title} />
							}
							transitionTime={100}
						>
							{Object.entries(query.data).map((e) => (
								// e : [ key, value ]
								<JsonView
									key={e[0]}
									graphqlUid={e[0]}
...
const DevModeItem = ({ isOpen, isGraphql, maxStringSize, nbLinkedDoc, title }) => (
