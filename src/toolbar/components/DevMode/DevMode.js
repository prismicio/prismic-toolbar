...
export class DevMode extends Component {
  ...
  render() {
    ...
    return (
      <div>
        {
          apiQueries.map(query => { // apiQuery : [Object]
        ...
        {
          graphqlApiQueries.map(query => { // graphqlQuery : {data: Object}
            if (!query.data || Object.keys(query.data).length < 1) { return null; }
            ...
            return (
              <Collapsible
                ...
                transitionTime={100}>
                {
                  Object.entries(query.data).map(e => // e : [ key, value ]
                    <JsonView
                      graphqlUid={e[0]}
                      isGraphql
                      json={e[1]}
...
const DevModeItem = ({ isOpen, isGraphql, maxStringSize, nbLinkedDoc, title }) => (
  <div className='wrapper-trigger'>
    ...
    <h3 className='trigger-subtitle'>{ isGraphql ? '[Graphql Query]' : nbLinkedDoc + ' linked documents'}</h3>
